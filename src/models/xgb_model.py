"""Modelo XGBoost para predicción de precio de cierre (t+1)."""
import sys
...
from src.models.rf_model import FEATURE_COLS, TARGET_COL, _prepare_xy


def _optimize_hyperparams(X_train, y_train, X_val, y_val, n_trials=N_OPTUNA_TRIALS):
    """Optimización bayesiana (Optuna) de hiperparámetros de XGBoost."""
...
def train_and_predict(ticker: str, optimize: bool = True):
    """Entrena XGBoost para un ticker, predice sobre el set de validación y guarda resultados."""
    ...
    save_predictions(ticker, "XGBoost", val_df["target_date"], pred_prices, actual_prices)
    save_metrics(ticker, "XGBoost", metrics)
    ...
    if future:
        ...
        save_predictions(ticker, "XGBoost", future_dates, future_prices)

    print(f"[XGBoost][{ticker}] {metrics} best_params={best_params}")
